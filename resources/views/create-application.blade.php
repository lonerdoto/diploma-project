...
@section('content')
            ...
            <form class="form-control px-10 py-12 bg-gray-100 rounded-3xl shadow-2xl flex flex-col justify-between gap-5"
                ...
                <div class="custom-select-wrapper">
                    <div class="relative custom-select border border-slate-200 rounded-xl">
                        ...
                        <div class="custom-select-trigger text-slate-500 rounded-xl bg-gray-100">
                            Выберите тип заявки *
                        </div>
                        <div class="custom-options bg-gray-100">
                            <input type="hidden" class="hidden" name="application-type-id" id="application-type-id">
                            @forelse($applicationTypes as $applicationType)
                                <span class="custom-option" id="{{$applicationType->id}}"
                                      data-value="{{$applicationType->type}}">{{$applicationType->type}}</span>
                            @empty
                                <span class="custom-option" data-value="none">Нет типов заявок</span>
                            @endforelse
                        </div>
                    </div>
                </div>
                <input type="hidden" name="type" id="selected_option">
                <div class="gap-3 form-group relative">
                    <textarea name="description"
                              class="bg-gray-100 resize-none px-12 py-2 w-full rounded-xl border border-slate-200" rows="4"
                              placeholder="Ваше сообщение *" oninput="deleteInvalidOnInput(this)"></textarea>
                    <svg class="second-color absolute top-3 left-3" width="23" height="23" fill="none"
                         xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M6.708 8.625h9.584m-9.584 3.833h9.584m3.833 6.709-3.186-1.593c-.241-.121-.362-.181-.489-.224a1.923 1.923 0 0 0-.345-.082c-.132-.018-.267-.018-.537-.018H5.942c-1.074 0-1.61 0-2.02-.209a1.916 1.916 0 0 1-.838-.838c-.209-.41-.209-.946-.209-2.02V6.9c0-1.073 0-1.61.209-2.02.184-.36.477-.654.838-.838.41-.209.946-.209 2.02-.209h11.116c1.074 0 1.61 0 2.02.21.361.183.654.476.838.837.209.41.209.947.209 2.02v12.267Z"
                            stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                </div>
                <div class="gap-3 form-group relative rounded-xl" name="file-input">
                    <label class="px-12 py-2 w-full rounded-xl border border-slate-200"
                           style="cursor: pointer; color: var(--first-color); : 3px solid black" for="files" name="file-label">
                        <span id="file-name">Выберите файлы</span>
                    </label>
                    <input name="files[]" multiple style="display: none" type="file" id="files">
                    <svg class="second-color absolute top-3 left-3" width="23" height="20" fill="none"
                         xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M16.292 14.167h.01m-1.352-2.5h2.3c.893 0 1.34 0 1.692.127.47.169.843.493 1.037.901.146.307.146.695.146 1.472 0 .776 0 1.165-.146 1.47-.194.41-.568.734-1.037.903-.352.127-.799.127-1.692.127H5.75c-.893 0-1.34 0-1.692-.127-.47-.17-.843-.494-1.037-.902-.146-.306-.146-.695-.146-1.471 0-.777 0-1.165.146-1.472.194-.408.568-.732 1.037-.902.352-.126.799-.126 1.692-.126h2.3m3.45.833V3.333m0 0 2.875 2.5m-2.875-2.5-2.875 2.5"
                            stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                </div>
                <a class="second-color text-xl" href="{{route('applications')}}">Мои заявки</a>
                <button class="bg-first items-center flex flex-col rounded-xl p-1">
                    <svg class="text-white" width="30" height="30" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M21.25 9.063a.937.937 0 0 0-.938.937v10a.938.938 0 0 0 1.875 0V10a.937.937 0 0 0-.937-.938ZM7.813 15c0 .518.42.938.937.938h6.487l-2.15 2.15a.937.937 0 0 0 1.326 1.325l3.75-3.75a.938.938 0 0 0 0-1.326l-3.75-3.75a.937.937 0 0 0-1.326 1.326l2.15 2.15H8.75a.937.937 0 0 0-.938.937Z"
                            fill="currentColor"/>
                        <path fill-rule="evenodd" clip-rule="evenodd"
                              d="M15.51 2c2.885 0 5.146 0 6.91.237 1.806.243 3.231.75 4.35 1.868 1.118 1.119 1.625 2.544 1.868 4.35.237 1.764.237 4.025.237 6.91v.144c0 2.886 0 5.147-.237 6.911-.243 1.806-.75 3.231-1.868 4.35-1.119 1.118-2.544 1.625-4.35 1.868-1.764.237-4.025.237-6.91.237h-.144c-2.886 0-5.147 0-6.911-.237-1.806-.243-3.231-.75-4.35-1.868-1.118-1.119-1.625-2.544-1.868-4.35C2 20.656 2 18.395 2 15.51v-.144c0-2.886 0-5.147.237-6.911.243-1.806.75-3.231 1.868-4.35C5.224 2.987 6.65 2.48 8.455 2.237 10.219 2 12.48 2 15.365 2h.144Zm9.934 3.431c-.712-.712-1.675-1.12-3.273-1.336-1.625-.218-3.76-.22-6.733-.22s-5.109.002-6.734.22c-1.598.215-2.56.624-3.273 1.336-.712.712-1.12 1.675-1.335 3.273-.219 1.625-.221 3.76-.221 6.733s.002 5.109.22 6.734c.215 1.598.624 2.56 1.336 3.273.712.712 1.675 1.12 3.273 1.335 1.625.219 3.76.221 6.733.221s5.109-.002 6.734-.22c1.598-.215 2.56-.624 3.273-1.336.712-.712 1.12-1.675 1.335-3.273.219-1.625.221-3.76.221-6.733s-.002-5.109-.22-6.734c-.215-1.598-.624-2.56-1.336-3.273Z"
                              fill="currentColor"/>
                    </svg>
                </button>
            </form>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const hiddenInputId = document.getElementById('application-type-id');

            trigger.addEventListener('click', function () {
                customSelect.classList.toggle('open');
                trigger.classList.remove('is-invalid')
                deleteInvalidOnInput(hiddenInput)
            });
    ...
    @include('components.scripts.showFilesNameOnUpload')
